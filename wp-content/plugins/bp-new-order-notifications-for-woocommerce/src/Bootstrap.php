...
class Bootstrap {
	...
	public function __construct() {
		...
		add_action( 'init', array( $this, 'int_event' ) );
		...
		self::add_ajax( 'bp_new_order_notification', array( $this, 'ajax_callback' ) );
		self::add_ajax( 'bp_new_order_offline_notification', array( $this, 'ajax_offline_callback' ) );
	}
	...
	public function random_greetings() {
		// code...
		$gretings = array( 'Woohoo', 'Howdy', 'Bravo', 'Wow', 'Ahoy', 'Yay', 'Yikes', 'Hooray', 'Whoa', 'Woot', 'Oh joy', 'Yo!', 'Viola!' );
	...
	public function enqueue_admin_script( $hook ) {
		...
		$params        = array(
			...
			'notifictaion_daily_sound' => plugin_dir_url( __DIR__ ) . 'assets/sounds/' . self::get_option( 'notifictaion_sound', 'sound_cha-ching' ) . '.mp3',

		);

		wp_enqueue_style( 'new-order-admin', BPNON_ASSETS . '/css/admin.css', array(), BPNON_PLUGIN_VER );
		wp_enqueue_script( 'datatable', BPNON_ASSETS . '/js/jquery.dataTables.min.js', array( 'jquery' ), null, true );
		if ( $currentScreen->id == 'woocommerce_page_bp_notification_order' ) {
			...
			wp_enqueue_script( 'datatable-ui', BPNON_ASSETS . '/js/dataTables.semanticui.min.js', array( 'jquery' ), null, true );
		}
	...
	public function int_event() {
		...
		add_action( 'bpnon_refresh_timer', array( $this, 'check_for_new_order' ) );
	}
